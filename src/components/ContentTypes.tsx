...
export const fetchContentTypes = server$(
  async (spaceId: string, environmentId: string) => {
...
export default component$(({ state }: { state: IState }) => {
  const contentTypesResource = useResource$<{ id: string; name: string }[]>(
    async ({ track }) => {
      const sourceSpaceId = track(() => state.source.spaceId);
      const sourceEnvironmentId = track(() => state.source.environmentId || '');
      const targetSpaceId = track(() => state.target.spaceId);
      const targetEnvironmentId = track(() => state.target.environmentId || '');
